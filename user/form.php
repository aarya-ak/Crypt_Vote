<?php
include("../header_inner.php");
include("table.php");
error_reporting(0);

if($_REQUEST['a']=="error")
{
	echo "<script>alert('Insert Faild!!!!')</script>";
}
if($_REQUEST['a']=="1")
{
	echo "<script>alert('Insert Sucessfully')</script>";
}
$k=0;
?>
<!DOCTYPE html>
<html lang="en">
<head>
 
  <meta charset="utf-8">
  <meta name="viewport" content="width=device-width, initial-scale=1">
  <link rel="stylesheet" href="bootstrap.min.css">
  <script src="jquery.min.js"></script>
  <script src="bootstrap.min.js"></script>
<style>
.error
  {
	  color:#F00 !important;
  }
</style>
</head>
<body>
<?php
include("data_validation.php");
include("../connection.php");
$g=0;
$result = mysqli_query($con,"SHOW FIELDS FROM $table");
$i = 0;
echo "<div class='col-sm-12'>";
echo "<h2><center><b>SUBADMIN REGISTRATIONS</b></center></h2>";
echo "<form action='insert.php' method='post' enctype='multipart/form-data' name='register_form' id='register_form'>";
while ($row = mysqli_fetch_array($result))
 {
  $name=$row['Field'];
  $type=$row['Type'];
  $type = explode("(", $type);
  $type_only=$type[0];
$i++;
$g++;
if($i==1)
{
	//$gender=enum("male");
	//echo"<td>Male <input type='radio' name='$name'> </td>";
}
elseif($name=='admin_id')
  {

     $sql2 = "select MAX(admin_id)+1 as aid  from user ";
     $result2 = mysqli_query($con, $sql2);
     $row22=mysqli_fetch_array($result2);
     echo "
	   <div class='col-md-6'>
                      <div class='form-group'>
     <label>".str_replace('_', ' ', $name)."<span style='color:red;'>*</span></label>";
     echo"<input type='text' value='' name='$name' class='form-control' required pattern='[0-9]{5}'> </div></div>";
  }
elseif($name=="admin_name")
  {
	  echo "
	  <div class='col-md-6'>
                      <div class='form-group'>
    <label>".str_replace('_', ' ', $name)."<span style='color:red;'>*</span></label>";
    echo"<input type='text' name='$name'class='form-control' required> </div></div>";
  }
else if($name=="adhar_no" )
  {
	  echo "
	  <div class='col-md-6'>
                        <div class='form-group'>
    <label>".str_replace('_', ' ', $name)."<span style='color:red;'>*</span></label>";
    echo"<input type='text' name='$name'class='form-control' required	pattern='[0-9]{8}'>
    </div></div>";
  }
elseif($name=="address")
  {
	  echo "
	  <div class='col-md-6'>
                      <div class='form-group'>
    <label>".str_replace('_', ' ', $name)."<span style='color:red;'>*</span></label>";
    echo"<input type='text' name='$name'class='form-control' required> </div></div>";
  }
else if($name=="mail_id" )
  {
	  echo "
	 <div class='col-md-6'>
                       <div class='form-group'>
   <label>".str_replace('_', ' ', $name)."<span style='color:red;'>*</span></label>";
   echo"<input type='text' name='$name'class='form-control' required></div></div>";
  }
elseif($name=='password')
  {
    $pp=rand(102000,999999);
    echo "
	  <div class='col-md-6'>
                       <div class='form-group'>
    <label>".str_replace('_', ' ', $name)."</label>";
    echo"<input type='password' value='$pp' name='$name'class='form-control' required	 readonly> 
    </div></div>";
  }
}
echo "
<div class='col-md-12'>
             <div class='col-md-3'><div class='form-group'>
			 <input type='submit' value='Submit' name='submit' class='form-control btn-success'>";
echo "</form>";
echo "
</div></div><div class='col-md-3'><div class='form-group'>
<form action='select.php' method='post'><input type='submit' name='view' value='View All' class='form-control btn-primary'></form>
</div></div>
<div class='clearfix'></div></div>";
mysqli_free_result($result);
echo "</div>
<div class='clearfix'></div>";
mysqli_close($con);
include("../footer_inner.php");
?>
   <div id="sample">
</body>
</html>