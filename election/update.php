...
$i = 0;
echo "<h2><center><b>UPDATE ELECTION STATUS</b></center></h2>";
...
elseif($type_only=="varchar" || $type_only=="int" || $type_only=="int" || $type_only=="tinyint"  )
  {
    ...
    <input type='text' name='$name' value='$datas' class='form-control' > 
    </div></div>";
	}
  elseif($type_only=="date" )
  {
	  echo "<div class='col-sm-2'>".str_replace('_', ' ', $name)."</div><div class='col-sm-4'>
    <input type='text' name='$name' id='t$k' value='$datas' class='form-control'></div>";
	  ?>
...
<form action='select.php' method='post'><input type='submit' name='view' value='Back' class='form-control btn-primary'>
</form></div></div><div class='clearfix'></div></div>";
